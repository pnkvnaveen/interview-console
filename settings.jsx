...
/* eslint-disable */
function SettingsScreen({ data, savedData, onSave, onClose }) {
  const [draft, setDraft] = useState(() => window.clone(data));
  const [saved, setSaved] = useState(false);
  const [deleteTopicModal, setDeleteTopicModal] = useState(null); // { ti, topic }
  ...
  const requestDelTopic = (ti) => {
    const topic = draft[ti];
    if (topic.questions.length > 0) {
      setDeleteTopicModal({ ti, topic });
    } else {
      mutate(d => d.splice(ti, 1));
    }
  };

  const confirmDelTopic = () => {
    const { ti } = deleteTopicModal;
  ...
  const resetToSaved = () => {
    setDraft(window.clone(savedData || data));
    setSaved(false);
  };
  ...
  return (
    <div className="page-scroll fade-in">
      <div className="page-wrap wide">
        ...
        <p className="page-lede" style={{ maxWidth: 'none' }}>
          ...
          {' '}Currently {draft.length} topics &middot; {totalQ} questions.
        </p>
        ...
        {draft.map((topic, ti) => (
          <div className="set-topic" key={topic.id}>
            <div className="set-topic-head">
              ...
              <button className="icon-btn danger" title="Delete topic" onClick={() => requestDelTopic(ti)}><Icon name="trash" size={16} /></button>
            </div>
        ...
        <div className="set-actions">
          ...
          <button className="btn-secondary" onClick={resetToSaved}><Icon name="rotate" size={16} /> Reset to saved</button>
          <button className="btn-secondary" onClick={handleDiscard}><Icon name="arrowLeft" size={16} /> Discard &amp; close</button>
      ...
      {deleteTopicModal && (
        <ConfirmModal
          title="Delete topic?"
          message={`"${deleteTopicModal.topic.name}" has ${deleteTopicModal.topic.questions.length} question${deleteTopicModal.topic.questions.length !== 1 ? 's' : ''}. Deleting this topic will also remove all its questions. This cannot be undone.`}
          confirmLabel="Delete topic"
          danger
          onConfirm={confirmDelTopic}
          onCancel={() => setDeleteTopicModal(null)}
        />
      )}
      ...
      {discardModal && (
        <ConfirmModal
          ...
          danger
          onConfirm={() => { setDiscardModal(false); onClose(); }}
